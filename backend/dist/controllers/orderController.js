...
const createOrder = async (req, res) => {
    try {
        const { customerId, discount = 0, paymentMethod = 'CASH', orderSource = 'WEB', notes, orderItems } = req.body;
        ...
        const order = await db_1.default.$transaction(async (tx) => {
            ...
            for (const item of orderItems) {
                ...
                // Calculate pricing
                const price = item.price !== undefined ? new client_1.Prisma.Decimal(item.price) : product.price;
            ...
            const discountDecimal = new client_1.Prisma.Decimal(discount);
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        const isTransitioningToRestock = (uppercaseStatus === 'CANCELED' || uppercaseStatus === 'RETURNED') &&
